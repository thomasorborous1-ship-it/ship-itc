#!/usr/bin/env bash
#
# DSP-1 v2 - reimplementacao com matematica do trabalho de RE publica
# (Overload / Neviksti / The Dumper / Andreas Naive, 2006).
#
# Otimizado para PS2: somente inteiros 16/32 bits, sem float, sem
# alocacao dinamica.  Tabelas SinTable/MulTable/DataRom embarcadas
# (3.5 KB total, cabe em cache).
#
# Aplicar (idempotente):
#   cd ~/SNESticleRevive
#   bash apply_dsp1_v2.sh
#   make clean && make
#
set -e

if [ ! -d src/snes/core ]; then
    echo "ERRO: rode na raiz do SNESticleRevive (precisa achar src/snes/core/)."
    exit 1
fi

H_FILE="src/snes/core/sndsp1.h"
C_FILE="src/snes/core/sndsp1.cpp"

# backups (so na primeira execucao, depois deixa intactos)
[ -f "${H_FILE}.orig" ] || cp -p "$H_FILE" "${H_FILE}.orig"
[ -f "${C_FILE}.orig" ] || cp -p "$C_FILE" "${C_FILE}.orig"

cat > "$H_FILE" << 'DSP1_HEADER_EOF'
/*
...
echo "ok: $H_FILE atualizado"
echo "ok: $C_FILE atualizado"
echo
echo "Tamanhos:"
ls -la "$H_FILE" "$C_FILE"
echo
